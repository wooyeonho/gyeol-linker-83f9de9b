/**
 * GYEOL BYOK — API 키 암호화/복호화 (서버 전용)
 */
const ALGO = 'AES-GCM';
const KEY_LEN = 256;
const IV_LEN = 12;
...
function getKey(): Promise<CryptoKey | null> {
  ...
  if (!secret || secret.length < 32) return Promise.resolve(null);
...
export const SUPPORTED_PROVIDERS = ['openai', 'anthropic', 'deepseek', 'groq', 'gemini', 'custom'] as const;

...
const MOCK_PROGRESS: ProgressCondition[] = [
  { label: '대화 수', icon: '💬', current: 67, required: 100, met: false },
  { label: '대화 주제', icon: '📚', current: 12, required: 15, met: false },
  { label: '기억 수', icon: '🧠', current: 25, required: 30, met: false },
  { label: '친밀도', icon: '💜', current: 45, required: 40, met: true },
  { label: '연속 접속', icon: '🔥', current: 5, required: 7, met: false },
];
...
export function EvolutionProgress({ isOpen, onClose, currentGen, onEvolve }: EvolutionProgressProps) {
  const targetGen = Math.min(currentGen + 1, 5);
  const allMet = MOCK_PROGRESS.every((c) => c.met);

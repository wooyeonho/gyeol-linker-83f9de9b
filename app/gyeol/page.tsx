...
import { ChatInterface } from '@/components/gyeol/ChatInterface';
import { EvolutionCeremony } from '@/components/gyeol/EvolutionCeremony';
import { useGyeolStore } from '@/store/gyeol-store';

const VoidCanvas = dynamic(() => import('@/components/gyeol/VoidCanvas'), { ssr: false });
...
export default function GyeolPage() {
  ...
  const initAgent = useCallback(async () => {
    ...
    try {
      let res = await fetch(`/api/gyeol/agent?userId=${encodeURIComponent(DEMO_USER_ID)}`);
      if (res.status === 404) {
        res = await fetch('/api/gyeol/agent', {
          method: 'POST',
      ...
      if (res.ok) {
        ...
        setAgent(data);
        const convRes = await fetch(`/api/gyeol/conversations?agentId=${data.id}&limit=50`);
        if (convRes.ok) {

...
export default function GyeolSocialPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await fetch(`/api/gyeol/social/matches?agentId=${agent.id}&limit=10`);
      if (res.ok) {
    ...
    <main className="min-h-screen bg-black text-[#E5E5E5] p-6 pb-24">
      <div className="max-w-md mx-auto space-y-6">
        ...
        <nav className="flex justify-center gap-6 text-sm text-white/50">
          <Link href="/gyeol" className="hover:text-white">홈</Link>
          <Link href="/gyeol/social" className="text-indigo-400">발견</Link>
          <Link href="/gyeol/settings" className="hover:text-white">설정</Link>
        </nav>
      </div>
    </main>
  );

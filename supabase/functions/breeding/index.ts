  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!agents || agents.length !== 2) {
        return new Response(JSON.stringify({ eligible: false, reason: "에이전트를 찾을 수 없습니다" }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
      }
      const a1 = agents.find((a: any) => a.id === agent1Id)!;
      const a2 = agents.find((a: any) => a.id === agent2Id)!;
      if (a1.gen < MIN_GEN || a2.gen < MIN_GEN) {
        return new Response(JSON.stringify({ eligible: false, reason: `번식하려면 Gen ${MIN_GEN} 이상이어야 합니다` }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
      }
      return new Response(JSON.stringify({ eligible: true, reason: "번식 가능" }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
    }
    ...
    if (!match || match.compatibility_score < MIN_COMPAT) {
      return new Response(JSON.stringify({ success: false, message: `호환도 ${MIN_COMPAT}% 이상 필요 (현재: ${match?.compatibility_score ?? 0}%)` }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
    }
    ...
    if (recentBreeding) {
      ...
      if (hoursSince < COOLDOWN_HOURS) {
        return new Response(JSON.stringify({ success: false, message: `쿨다운 중 (${Math.ceil(COOLDOWN_HOURS - hoursSince)}시간 남음)` }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
      }
    }
    ...
    if (roll >= SUCCESS_RATE) {
      ...
      return new Response(JSON.stringify({ success: false, message: `번식 실패 — 다음에 다시 시도하세요` }), { headers: { ...corsHeaders(req), "Content-Type": "application/json" } });
    }
